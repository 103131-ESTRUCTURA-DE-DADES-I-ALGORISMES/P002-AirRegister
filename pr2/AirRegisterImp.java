...
public class AirRegisterImp implements AirRegister {    
    ...
    public boolean addCompany(Company c) {
        if(companies.containsKey(c)) return false;
    ...
    public boolean registerAircraft(Company c, Aircraft a) {
    	...
    	if(t!=null && !t.equals(c)) throw new DifferentCompanyException("Aquest avió ja està registrat en unaltre companyia");
        if (companies.containsKey(c)) return companies.get(c).add(a);
    ...
    public Company findCompany(AircraftID id) {
    	for (Map.Entry<Company, TreeSet<Aircraft>> contract: companies.entrySet()) {
    		if(contract.getValue().stream().anyMatch(a->a.getId().equals(id))) return contract.getKey();
    	}
    	return null; //Solucio massa OP: return companies.entrySet().stream().filter(e->e.getValue().stream().anyMatch(a->a.getId().equals(id))).map(e->e.getKey()).findFirst().orElse(null);
    }
    @Override
